...
let get s n = try Some (Array.get s n) with _ -> None
let sub s n l = try Some (Array.sub s n l) with _ -> None
...
let decode s =
  let result = Array.make (String.length s / 4 * 3) '\x00' in
  let rec inner i acc =
    match (getchr s i, i mod 4) with
    | Some -1, _ -> inner (i + 1) (acc + 1)
    | Some c, 0 ->
        (* XXXXXX-- -------- -------- *)
        result.(i / 4 * 3) <- chr (c lsl 2);
        inner (i + 1) acc
    | Some c, 1 ->
        (* ------XX XXXX---- -------- *)
        result.(i / 4 * 3) <- chr (Char.code result.(i / 4 * 3) lor (c lsr 4));
        result.((i / 4 * 3) + 1) <- chr (c lsl 4);
        inner (i + 1) acc
    | Some c, 2 ->
        (* -------- ----XXXX XX------ *)
        result.((i / 4 * 3) + 1) <-
          chr (Char.code result.((i / 4 * 3) + 1) lor (c lsr 2));
        result.((i / 4 * 3) + 2) <- chr (c lsl 6);
        inner (i + 1) acc
    | Some c, 3 ->
        (* -------- -------- --XXXXXX *)
        result.((i / 4 * 3) + 2) <-
  ...
  else Array.sub result 0 (Array.length result - ignored_chars)
